...
export default function AdminProgramFilters({ value, onChange, includeSession = true, includeStatus = false, statusOptions = [], includeLevel = false, includeLanguage = false, className = '' }) {
  ...
  const fieldCount = 2 + (includeSession ? 1 : 0) + (includeStatus ? 1 : 0) + (includeLevel ? 1 : 0) + (includeLanguage ? 1 : 0);
