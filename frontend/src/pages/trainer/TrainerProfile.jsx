...
import { CameraIcon, CheckIcon, PencilIcon, UserCircleIcon, XMarkIcon } from '@heroicons/react/24/outline';
import api from '../../api/axios';
import PhoneInput from '../../components/common/PhoneInput';

const TITLES = ['Mr.', 'Ms.', 'Mrs.', 'Dr.', 'Prof.', 'Eng.'];
const photoUrl = (path) => path ? (path.startsWith('http') ? path : `/${path.replace(/^\//, '')}`) : null;
const toForm = (trainer) => ({ title: trainer.title || '', name: trainer.name || '', phone: trainer.phone || '', organization: trainer.organization || '', expertise: (trainer.expertise || []).join(', '), biography: trainer.biography || '' });
