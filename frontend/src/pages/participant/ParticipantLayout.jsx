...
const ParticipantLayout = () => {
  ...
  return <div className="min-h-screen bg-slate-100 md:flex">
    ...
    <nav aria-label="Participant mobile navigation" className="participant-bottom-nav border-t border-slate-200 bg-white/95 pt-1.5 shadow-[0_-8px_30px_rgba(15,23,42,.08)] backdrop-blur-md md:hidden"><div className="participant-bottom-nav-grid">{items.map((item) => { const Icon = item.icon; const isActive = active(item.path); return <Link key={item.path} to={item.path} aria-current={isActive ? 'page' : undefined} className={`flex min-h-16 min-w-0 flex-col items-center justify-center gap-1 px-0.5 py-1 text-[11px] font-bold leading-tight transition-colors ${isActive ? 'text-[#1da156]' : 'text-black'}`}><span className="flex h-8 w-10 items-center justify-center"><Icon className="h-6 w-6" /></span><span className="w-full text-center">{item.name}</span></Link>; })}</div></nav>
  </div>;
};
