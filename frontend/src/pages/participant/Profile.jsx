...
import { CameraIcon, CheckIcon, EyeIcon, EyeSlashIcon, KeyIcon, PencilIcon, ShieldCheckIcon, XMarkIcon } from '@icons';
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <form onSubmit={handleProfileSubmit} className="space-y-6 p-6 sm:p-8">
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
          <label>Full name *<input className={`${inputClass} mt-2`} value={profileForm.fullName} placeholder="e.g. Amina Mohamed Ali" onChange={(e) => setProfileForm({ ...profileForm, fullName: e.target.value })} required /></label>
          ...
          <label>University / School<input className={`${inputClass} mt-2`} value={profileForm.organization} placeholder="e.g. Hormuud University" onChange={(e) => setProfileForm({ ...profileForm, organization: e.target.value })} /></label>
          <label>Profession<input className={`${inputClass} mt-2`} value={profileForm.profession} placeholder="e.g. Software Engineer" onChange={(e) => setProfileForm({ ...profileForm, profession: e.target.value })} /></label>
        </div>
        <button disabled={profileSubmitting} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#1a6b3c] px-6 text-sm font-bold text-white disabled:opacity-60">{profileSubmitting ? <ButtonSpinner /> : <CheckIcon className="h-5 w-5" />}{profileSubmitting ? 'Saving…' : 'Save profile'}</button>
      </form>}
    </section>
