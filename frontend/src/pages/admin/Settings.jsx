...
import { EnvelopeIcon, EyeIcon, EyeSlashIcon, GlobeAltIcon, PencilIcon, PhotoIcon, ShieldCheckIcon, TrashIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const initial = { organizerName: '', contactEmail: '', replyToEmail: '', location: '', facebookUrl: '', emailSenderName: '', smtpUser: '', smtpPassword: '', hasSmtpPassword: false, certificateSignature: '', certificateSignatoryName: 'Authorized Signatory', certificateSignatoryTitle: 'National Training Week' };
...
const Settings = () => {
  ...
  return <div className="mx-auto max-w-5xl space-y-6 pb-8">
    ...
    <form onSubmit={save} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      <section className="p-6 sm:p-8"><div className="mb-6 flex items-start gap-4"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-emerald-50 text-[#1a6b3c]"><GlobeAltIcon className="h-5 w-5" /></span><div><h2 className="font-bold text-slate-950">Public information</h2><p className="mt-1 text-sm text-slate-500">These details appear in the public website footer.</p></div></div><div className="grid grid-cols-1 gap-5 sm:grid-cols-2"><label className="text-sm font-semibold text-slate-700">Organizer name *<input name="organizerName" value={form.organizerName} onChange={change} className={inputClass} disabled={!editing} required /></label><label className="text-sm font-semibold text-slate-700">Location *<input name="location" value={form.location} onChange={change} className={inputClass} disabled={!editing} required /></label><label className="text-sm font-semibold text-slate-700">Public contact email *<input type="email" name="contactEmail" value={form.contactEmail} onChange={change} placeholder="contact@example.org" className={inputClass} disabled={!editing} required /></label><label className="text-sm font-semibold text-slate-700">Facebook page URL<input type="url" name="facebookUrl" value={form.facebookUrl} onChange={change} placeholder={editing ? 'https://facebook.com/your-page' : 'Not configured'} className={inputClass} disabled={!editing} /><span className="mt-1.5 block text-xs font-normal text-slate-400">Leave blank to hide the Facebook icon.</span></label></div></section>
