    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-xs space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          >
            {bulkSubmitting ? <><ButtonSpinner /> Queuing Certificates...</> : 'Run Bulk Certificate Generator'}
          </button>
        </div>
      ...
      {revokeTarget && <div role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setRevokeTarget(null); }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4">
        <section role="dialog" aria-modal="true" aria-labelledby="revoke-title" className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
          ...
          <form onSubmit={handleRevoke} className="mt-5"><label htmlFor="revoke-reason" className="text-sm font-bold text-slate-800">Revocation reason</label><textarea id="revoke-reason" autoFocus required minLength={5} maxLength={300} rows={4} value={revokeReason} onChange={(event) => setRevokeReason(event.target.value)} placeholder="e.g. Issued using an incorrect participant record" className="mt-2 w-full rounded-xl border border-slate-300 p-3 text-sm outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-100" /><div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={() => setRevokeTarget(null)} className="min-h-11 rounded-xl border border-slate-300 px-5 text-sm font-bold text-slate-700">Cancel</button><button type="submit" disabled={revoking} className="min-h-11 rounded-xl bg-rose-600 px-5 text-sm font-bold text-white disabled:opacity-60">{revoking ? <><ButtonSpinner /> Revoking…</> : 'Revoke certificate'}</button></div></form>
        </section>
      </div>}
