...
export const RegistrationsManagement = () => {
  ...
  const updateTraining = async (id, status) => {
    setBusy(`${id}:${status}`);
    try { await api.patch(`/admin/registrations/${id}/status`, { status }); toast.success(`Training registration ${status}.`); await fetchData(); }
  ...
  const deleteRegistrations = async (ids) => {
    ...
    finally { setBusy(''); setBulkDeleting(false); }
  };
  ...
  return <div className="space-y-6">
    ...
    <div className="flex flex-wrap gap-2"><button type="button" onClick={() => setAnnouncementOpen(true)} disabled={!filters.training} title={!filters.training ? 'Select a training session first' : 'Email pending and approved applicants'} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-[#1a6b3c] px-3 text-xs font-bold text-white hover:bg-[#145731] disabled:cursor-not-allowed disabled:opacity-40"><PaperAirplaneIcon aria-hidden="true" className="h-4 w-4" />Email filtered applicants</button><button type="button" onClick={approveFiltered} disabled={approvingAll || !registrations.some((registration) => registration.status === 'pending')} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-emerald-600 px-3 text-xs font-bold text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-40">{approvingAll ? <ButtonSpinner /> : <CheckIcon aria-hidden="true" className="h-4 w-4" />}{approvingAll ? 'Approving...' : 'Approve all filtered'}</button><button type="button" onClick={() => deleteRegistrations(selectedIds)} disabled={!selectedIds.length || bulkDeleting} className="inline-flex items-center justify-center gap-2 rounded-lg bg-rose-600 px-3 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-40">{bulkDeleting && <ButtonSpinner size="xs" />}{bulkDeleting ? 'Deleting…' : 'Delete selected'}</button><button type="button" onClick={() => deleteRegistrations(registrations.map((item) => item._id))} disabled={!registrations.length || bulkDeleting} className="inline-flex items-center justify-center gap-2 rounded-lg border border-rose-200 px-3 py-2 text-xs font-bold text-rose-700 disabled:cursor-not-allowed disabled:opacity-40">{bulkDeleting && <ButtonSpinner size="xs" />}{bulkDeleting ? 'Deleting…' : 'Delete all'}</button></div>
    {loading ? <LoadingSpinner label="Loading training registrations..." /> : <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs"><div className="overflow-x-auto"><table className="w-full text-left text-xs">
      ...
      <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
        {registrations.map((reg) => <tr key={reg._id} className="hover:bg-slate-50"><td className="p-4"><input type="checkbox" checked={selectedIds.includes(reg._id)} onChange={() => toggleSelected(reg._id)} aria-label={`Select registration for ${reg.participant?.fullName || 'participant'}`} className="h-4 w-4 accent-[#1a6b3c]" /></td><td className="p-4"><strong className="block text-slate-950">{reg.participant?.fullName}</strong><span className="text-slate-400">{reg.participant?.email}</span></td><td className="p-4 font-bold text-[#1a6b3c]">{reg.training?.title}</td><td className="p-4">{new Date(reg.registeredAt).toLocaleDateString()}</td><td className="p-4"><StatusBadge status={reg.status} /></td><td className="p-4"><div className="flex gap-2">{reg.status !== 'approved' && <button onClick={() => updateTraining(reg._id, 'approved')} disabled={busy.startsWith(`${reg._id}:`)} className={`${actionClass} bg-emerald-600 hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-60`}>{busy === `${reg._id}:approved` ? <ButtonSpinner size="xs" /> : <CheckIcon className="h-4 w-4" />} {busy === `${reg._id}:approved` ? 'Approving…' : 'Approve'}</button>}{reg.status !== 'rejected' && <button onClick={() => updateTraining(reg._id, 'rejected')} disabled={busy.startsWith(`${reg._id}:`)} className={`${actionClass} bg-rose-600 hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-60`}>{busy === `${reg._id}:rejected` ? <ButtonSpinner size="xs" /> : <XMarkIcon className="h-4 w-4" />} {busy === `${reg._id}:rejected` ? 'Rejecting…' : 'Reject'}</button>}<button onClick={() => deleteRegistrations([reg._id])} disabled={busy.startsWith(`${reg._id}:`)} aria-label="Delete registration" title="Delete registration" className="rounded-lg p-2 text-slate-400 hover:bg-rose-50 hover:text-rose-600 disabled:cursor-not-allowed disabled:opacity-60">{busy === `${reg._id}:delete` ? <ButtonSpinner /> : <TrashIcon className="h-4 w-4" />}</button></div></td></tr>)}
