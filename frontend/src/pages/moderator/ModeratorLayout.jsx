...
export const ModeratorLayout = () => {
  ...
  return <div className="min-h-screen bg-slate-100 md:flex">
    {sidebarOpen && <button type="button" aria-label="Close moderator menu" className="fixed inset-0 z-40 bg-black/50 md:hidden" onClick={() => setSidebarOpen(false)} />}
    ...
    <div className="min-w-0 flex-1">
      <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur sm:px-6 lg:px-8"><div className="flex items-center gap-3"><button type="button" onClick={() => setSidebarOpen(true)} className="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-slate-200 text-slate-700 md:hidden" aria-label="Open moderator menu"><Bars3Icon className="h-6 w-6" /></button><div className="hidden sm:block"><p className="text-sm font-bold text-slate-950">Moderator Workspace</p><p className="text-xs text-slate-500">Session operations</p></div></div><div className="flex items-center gap-2"><ThemeToggle /><div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 sm:min-w-52"><div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#1a6b3c] text-sm font-bold text-white">{profilePhoto ? <img src={profilePhoto} alt="" className="h-full w-full object-cover" /> : (user?.fullName || 'M').charAt(0).toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-bold text-slate-950">{user?.fullName || 'Moderator'}</p>{user?.email && <p className="hidden truncate text-xs text-slate-500 sm:block">{user.email}</p>}</div></div></div></header>
