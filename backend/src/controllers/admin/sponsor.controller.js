...
const sponsorData = (req, existing = null) => ({
  ...
  category: req.body.category || 'Co-Organizer',
...
export const createSponsor = async (req, res, next) => {
  try {
    if (!req.file) return errorResponse(res, 'Co-organizer logo is required.', 400);
    const sponsor = await Sponsor.create({
      ...sponsorData(req),
      logo: `uploads/sponsorLogo/${req.file.filename}`,
      createdBy: req.user._id,
    });
    return successResponse(res, { sponsor }, 'Co-organizer added successfully.', 201);
  } catch (err) {
...
export const updateSponsor = async (req, res, next) => {
  try {
    ...
    if (!sponsor) {
      if (req.file) deleteFile(req.file.path);
      return errorResponse(res, 'Co-organizer not found.', 404);
    }
    ...
    return successResponse(res, { sponsor }, 'Co-organizer updated successfully.');
  } catch (err) {
    if (req.file) deleteFile(req.file.path);
    next(err);
  }
...
export const toggleSponsorStatus = async (req, res, next) => {
  try {
    ...
    if (!sponsor) return errorResponse(res, 'Co-organizer not found.', 404);
    return successResponse(res, { sponsor }, `Co-organizer ${sponsor.isActive ? 'published' : 'hidden'}.`);
  } catch (err) { next(err); }
};
...
export const deleteSponsor = async (req, res, next) => {
  try {
    ...
    return successResponse(res, null, 'Co-organizer deleted successfully.');
  } catch (err) { next(err); }
};
