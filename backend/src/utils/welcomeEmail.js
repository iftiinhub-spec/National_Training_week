...
export const sendWelcomeEmail = ({ to, participantName }) => sendEmail({
  ...
  html: emailLayout({
    ...
    body: `<p style="margin-top:0">Hello ${participantName || 'Participant'},</p><p>Welcome to National Training Week. Your participant account has been created successfully and is ready to use.</p>${emailInfoCard([['Account status', 'Active'], ['Training registration', 'Approval required for each session']])}<p>You can sign in now, explore the published training sessions, and request a place in the sessions you want to attend.</p>${emailButton('Sign in to participant portal', `${process.env.FRONTEND_URL}/signin`)}`,
  }),
});
